# CDM/DRR Research Assessment: Nomura Holdings, Inc.

## Executive Summary

**Bank:** Nomura Holdings, Inc.
**Classification:** OBSERVER (CCP-Connected)
**Confidence:** 45%
**Research Date:** 2025-12-21
**Phase:** 3 (Japanese Banks)

Nomura shows no evidence of direct CDM strategic adoption. The only CDM connection is through JSCC clearing membership, where JSCC deployed CDM production infrastructure in June 2025. This creates passive exposure to CDM but does not indicate internal CDM capabilities or strategic commitment.

## Evidence Summary

### Tier 1 Evidence: 0 items
Comprehensive searches of official sources yielded no evidence:
- No official CDM announcements
- No FINOS/open source contributions
- No regulatory filing mentions
- No annual report references

### Tier 2 Evidence: 2 items
1. JSCC clearing membership (indirect CDM exposure)
2. ISDA event sponsorship (general industry engagement)

### Tier 3 Evidence: 0 items
No hiring signals, expertise indicators, or technical publications

### Null Results: 8 informative absences
Consistent pattern across all evidence tiers indicating no CDM program

## Classification Rationale

Nomura is classified as OBSERVER (CCP-Connected) based on:
1. Indirect connection to JSCC CDM infrastructure
2. No evidence of internal CDM development
3. No hiring for CDM capabilities
4. No vendor partnerships announced
5. No trade press coverage

The CCP-Connected sub-classification acknowledges infrastructure exposure while distinguishing it from strategic adoption.

## Confidence Assessment

45% confidence reflects:
- No Tier 1 evidence (caps confidence per protocol)
- Single indirect Tier 2 connection
- Consistent null results increase certainty about "not ARCHITECT"
- Moderate uncertainty about future intent

## Business Context

**Organization Profile:**
- Major Japanese securities firm
- Significant global derivatives operations
- Strong investment banking presence

**CDM Drivers:**
- JSCC clearing requirements (regulatory mandate)
- Global counterparty expectations (moderate)
- Operational efficiency (low priority)

**CDM Hesitations:**
- High implementation costs
- Competing technology priorities
- Sufficient traditional infrastructure
- Wait-and-see approach common among Japanese banks

## Knowledge Gaps

**Critical Gaps:**
1. Internal JSCC connectivity implementation (vendor solution vs internal build?)
2. Strategic planning for CDM beyond JSCC compliance
3. Vendor relationships for derivatives reporting
4. Product-level CDM coverage (which products use JSCC?)

**Impact:** These gaps affect precision of sub-classification but not primary classification

## Conclusion

Nomura is an OBSERVER with infrastructure-mandated CDM exposure through JSCC. No evidence of strategic CDM adoption or internal capability building. Classification is stable and well-supported by consistent null results across all evidence tiers.

**Recommended Follow-Up:**
- Monitor for announcements following JSCC deployment maturation
- Watch for vendor partnership disclosures
- Track hiring signals for CDM expertise
- Review classification in 12 months as JSCC ecosystem develops
